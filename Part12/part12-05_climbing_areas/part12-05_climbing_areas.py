...
class ClimbingArea:
	...
	def add_route(self, route: ClimbingRoute) -> None:
		self.__routes.append(route)

	def routes(self):
		return len(self.__routes)

	def hardest_route(self) -> ClimbingRoute:
...
def sort_by_number_of_routes(routes: list):
	def number_of_route(route: ClimbingArea):
		return route.routes()

	return sorted(routes, key=number_of_route)


def sort_by_most_difficult(climb_areas: list):
	def areas(area: ClimbingArea):
		return area.hardest_route().grade
	return sorted(climb_areas, key=areas, reverse=True)
