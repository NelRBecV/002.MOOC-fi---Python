...
def cheaters():
	...
	for start in start_t:
		task_s = datetime.strptime(start[1], "%H:%M")

		for end in end_t:
			if start[0] == end[0]:
				task_f = datetime.strptime(end[3], "%H:%M")
				# If this student took more than 3 hours to finish an exercise, it is considered as he/she cheated on
				if (task_f - task_s).total_seconds() > 10800 and not start[0] in cheater:
					cheater.append(start[0])
	return cheater
